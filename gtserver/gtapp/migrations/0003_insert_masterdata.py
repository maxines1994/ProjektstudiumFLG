...
class Migration(migrations.Migration):
    ...
    def insert_tasktypes(apps, schema_editor):
        ...
        TaskType.objects.create(id=47   , title = "Bestellreklamation abgeschlossen"            , description ="Bitte montieren Sie die reklamierten und neugelieferten Teile. Sollte ein Teil erneut mangelhaft geliefert worden sein, so legen Sie bitte eine neue Bestellreklamation an."                                                , status_model = SuppComplaint.__name__     , task_model= SuppComplaint.__name__    , status=SuppComplaintDet.Status.ABGESCHLOSSEN                 , view_url = 'supp_complaint_alter'    , view_kwargs_id = 'supp_complaint_id'  , group=Group.objects.get(name=PRODUKTION))
