...
class Migration(migrations.Migration):
    ...
    def insert_tasktypes(apps, schema_editor):
        ...
        TaskType.objects.create(id=30   , title = "Auftragsreklamation freigeben"                , description ="Bitte geben Sie den Bestellreklamation frei."                                                                                                               , status_model = CustComplaint.__name__     , task_model= CustComplaint.__name__    , status=CustComplaint.Status.ERFASST                           , view_url = 'cust_complaint_alter'    , view_kwargs_id = 'cust_complaint_id'  , group=Group.objects.get(name=KUNDENDIENST))
        TaskType.objects.create(id=31   , title = "Hubwagen an Kunden senden"                   , description ="Bitte senden Sie den Hubwagen an den Kunden. Denken Sie daran, vorab die Barcodenummer per Nachricht zu schicken."                                                                                                               , status_model = CustComplaintDet.__name__  , task_model= CustComplaintDet.__name__ , status=CustComplaintDet.Status.BEI_KUNDENDIENST               , view_url = 'cust_complaint_alter'    , view_kwargs_id = 'cust_complaint_id'  , group=Group.objects.get(name=KUNDENDIENST))
