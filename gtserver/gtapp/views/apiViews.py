...
def get_api_tasks(request, **kwargs):
    ...
    for index, task in enumerate(Todo.get_tasks_of_user(request.user)):
        tasks_user.append(dict(id=task.id, title=task.todo_type.title, ref=task.get_ref()))

    tasks_group = list()
    for index, task in enumerate(Todo.get_unassigned_tasks(request.user)):
        tasks_group.append(dict(id=task.id, title=task.todo_type.title, ref=task.get_ref()))
